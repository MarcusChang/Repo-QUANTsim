...
'  Call MenuToolbarPopup.ClickItem("P&roduct Defaults", False)
  ...
  Call OptionViewGrid.Keys("~vr")
  
  Call TestUtilities.WaitUntilAliasVisible(Aliases.MarketView,"dlgProductDefaultsEditor",2000)
  
End Sub

Sub OK
  Dim dlgProductDefaultsEditor
  Set dlgProductDefaultsEditor = Aliases.MarketView.dlgProductDefaultsEditor
...
Sub ClickMachineTab(TabName)
  Dim MachineTab
  Set MachineTab = Aliases.MarketView.dlgProductDefaultsEditor.TabControl.General.MachineTab
...
Sub ACRestrictionSetLocation(ClickType,Location)
  ...
  Set AOMGrid = Aliases.MarketView.dlgProductDefaultsEditor.TabControl.General.MachineTab.Restrictions.AOMGrid
...
Sub SetAOM(SpecName)
  Dim  AOMComboBox
  Set  AOMComboBox = Aliases.MarketView.dlgProductDefaultsEditor.TabControl.General.MachineTab.General.AOM
  Call AOMComboBox.ClickItem(SpecName)
End Sub

Sub SetMQ(SpecName)
  Dim  MQComboBox
  Set  MQComboBox = Aliases.MarketView.dlgProductDefaultsEditor.TabControl.General.MachineTab.General.MQ
  Call MQComboBox.ClickItem(SpecName)
End Sub

Sub SetTOM(SpecName)
  Dim  TOMComboBox
  Set  TOMComboBox = Aliases.MarketView.dlgProductDefaultsEditor.TabControl.General.MachineTab.General.TOM
  Call TOMComboBox.ClickItem(SpecName)
End Sub

Sub SetTheoCheckType(Value)
  Dim  TheoCheckType
  Set  TheoCheckType = Aliases.MarketView.dlgProductDefaultsEditor.TabControl.General.TheoCheckType
  Call TheoCheckType.ClickItem(Value)
End Sub

Sub SetTheoCheckTicks(Value)
  Dim  TheoCheckTicks
  Set  TheoCheckTicks = Aliases.MarketView.dlgProductDefaultsEditor.TabControl.General.TheoCheckTicks
  Call TheoCheckTicks.Keys("[Home]![End][Del]"&Value) 
End Sub

Sub SetTheoCheckPercent(Value)
  Dim  TheoCheckPercent
  Set  TheoCheckPercent = Aliases.MarketView.dlgProductDefaultsEditor.TabControl.General.TheoCheckPercent
  Call TheoCheckPercent.Keys("[Home]![End][Del]"&Value)
End Sub

Sub SetTheoCheckWarning(Value)
  Dim  btnWarnings
  Set  btnWarnings = Aliases.MarketView.dlgProductDefaultsEditor.TabControl.General.btnWarnings
  Call btnWarnings.ClickButton
End Sub

Sub SetClickQuantity(Value)  
  Dim QuantityDefaults 
  Set QuantityDefaults = Aliases.MarketView.dlgProductDefaultsEditor.TabControl.General.BCGPGridCtrl40000081000710
  'Call Open
  'Call QuantityDefaults.Click(126, 30)
  Call QuantityDefaults.Click(100, 30)  
  Call QuantityDefaults.Edit.SetText(Value)
  Call OK
End Sub

Sub ClickQuantity(ProductPath,Qty)
                                  
    Call ProductDefaults.Open
    Call ProductDefaults.ClickProduct(ProductPath)
    Call ProductDefaults.SetClickQuantity(Qty)   
